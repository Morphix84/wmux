...
test("user service refreshes checkout dependencies before restarting", () => {
  const installer = read("scripts/install-user-service.sh");
  const permissionRepairAt = installer.indexOf('chmod 700 "${HOME}/.wmux"');
  const installAt = installer.indexOf('(cd "${ROOT_DIR}" && npm install)');
  const restartAt = installer.indexOf("systemctl --user restart wmux.service");
  assert.ok(permissionRepairAt >= 0);
  assert.ok(installAt > permissionRepairAt);
  assert.ok(restartAt > installAt);
});
