...
The gold `?` indicator is reserved for a positively identified explicit input request.
The managed Prime Agent extension marks an active `questionnaire` tool as waiting, keeps that state across overlapping root or nested questions, and returns to Working after the last questionnaire resolves; it does not guess that ordinary idle completion requires input.
...
On older Prime versions, a resident session stays bound to its creator pane; reopening a nonresident saved session can bind it to the new pane.
When Prime exposes foreground client context snapshots, the extension instead binds each turn to the validated client generation, allowing resident sessions to resume in a new pane.
Questionnaires, idle recovery, titles, and heartbeat callbacks retain that generation and reject stale callbacks after rebinding.
The process retains at most 1,024 client families; evicted families remain disabled until process restart, and the bounded retirement filter can also disable a new family after extreme session churn.
